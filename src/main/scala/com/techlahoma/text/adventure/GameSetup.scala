...
trait GameSetup {
  ...
  val computerNote2 = RoomObject('computerNote2, "parser combinator note", "There is a note about parser combinators here.")
    .withInteraction(Interaction.Read, (s:State, _) => s.withPrompt(ansi"In between %bold{regex} and more %bold{complicated techniques}, there is a gap: what about the cases where you don't need utmost performance, but your data format is too complex to use a %bold{regex}? ${"\n\n"}Why can't there be a way of writing parsers as simple as a %bold{regex}, but as flexible as %bold{hand-rolled recursive-descent parsers} or %bold{parser generators}? It turns out there are such things: they're called %bold{Parser Combinators}."))
